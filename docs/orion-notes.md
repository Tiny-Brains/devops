...
| **`db-init/` runs once per volume, so a pre-release schema rewritten in place strands every existing dev stack** | It surfaces as `relation "clocks" does not exist` on every count tick, which looks like a bug in count. `devops/scripts/resync-dev-schema.sh` is the answer, and it rebuilds from the migrations rather than maintaining a delta |
